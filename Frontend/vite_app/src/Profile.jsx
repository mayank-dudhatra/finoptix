...
export default function Profile() {
  ...
  useEffect(() => {
    fetch("https://jsonplaceholder.typicode.com/users/1")
      ...
      .then(data => setUserData(prev => ({
        ...
        username: data.username ? `@${data.username}` : prev.username,
        email: data.email || prev.email,
        location: `${data.address?.city || "Ahmedabad"}, ${data.address?.country || "India"}`,
      })))
      .catch(err => console.error("Error fetching data:", err))
  }, [])
  ...
  const handleEditToggle = async () => {
    if (isEditing) {
      const res = await fetch("https://jsonplaceholder.typicode.com/users/1", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(userData) })
  ...
  const handleImageUpload = setImage => e => { const file = e.target.files[0]; if (file) { const reader = new FileReader(); reader.onloadend = () => setImage(reader.result); reader.readAsDataURL(file) } }
  const toggleGoalCompletion = index => setGoals(prev => { const updated = [...prev]; updated[index].completed = !updated[index].completed; return updated })
    ...
    <div className="bg-[#F7FAFC] shadow-xl font-['Inter']">
        ...
        <div className="flex items-center justify-between w-full">
          ...
          <nav className="hidden md:flex items-center gap-x-[15px] flex-grow justify-center text-[20px] font-outfit font-normal tracking-[0.08em]">
            <motion.a href="/" className="px-4 flex items-center gap-x-[9px] py-2 rounded-md hover:bg-purple-700" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <img src={Home} alt="Finoptix Logo" className="w-[22px] h-[22px] object-contain pb-1" />Home
            </motion.a>
            <motion.a href="/dashboard" className="px-4 flex items-center gap-x-[9px] py-2 rounded-md hover:bg-purple-700" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <img src={Clock} alt="Finoptix Logo" className="w-[20px] h-[20px] object-contain pt-0.5" />Dashboard
            </motion.a>
            <motion.a href="/your_budget" className="px-4 flex items-center gap-x-[9px] py-2 rounded-md hover:bg-purple-700" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <img src={Wallet} alt="Finoptix Logo" className="w-[20px] h-[20px] object-contain pt-0.5" />Your Budget
            </motion.a>
            <motion.a href="/goals" className="px-4 flex items-center gap-x-[9px] py-2 rounded-md hover:bg-purple-700" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <img src={Goal} alt="Finoptix Logo" className="w-[20px] h-[20px] object-contain pt-0.5 text-[#4B0082]" />Goals
            </motion.a>
            <motion.a href="/analytics" className="px-4 flex items-center gap-x-[9px] py-2 rounded-md hover:bg-purple-700" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <img src={Graph} alt="Finoptix Logo" className="w-[20px] h-[20px] object-contain pt-0.5" />Investment Analytics
            </motion.a>
          </nav>
          <div className="flex items-center gap-x-[15px] relative left-[-30px]">
      ...
      >
        ...
        <motion.div {...fadeInUp} transition={{ delay: 0.2 }} className="absolute left-1/2 -translate-x-1/2 top-[120px]" onMouseEnter={() => setIsHoveringPhoto(true)} onMouseLeave={() => setIsHoveringPhoto(false)} onClick={e => { e.stopPropagation(); profileInputRef.current.click() }}>
          <div className="relative group cursor-pointer">
            <img src={User} alt="Profile" className="w-32 h-32 rounded-full border-4 border-white object-cover shadow-lg" />
      ...
      <div className="px-10 pt-16 pb-12">
        <motion.div {...fadeInUp} className="text-center mb-10">
          {isEditing ? (
            <>
              <input value={userData.fullName} onChange={e => handleInputChange("fullName", e.target.value)} className="text-4xl font-['Poppins'] font-bold text-[#2D3748] bg-transparent border-b border-[#E2E8F0] focus:outline-none focus:border-[#F6AD55] mb-1" />
              <input value={userData.username} onChange={e => handleInputChange("username", e.target.value)} className="text-lg font-['Inter'] text-[#718096] bg-transparent border-b border-[#E2E8F0] focus:outline-none focus:border-[#F6AD55]" />
            </>
          ) : (
            <>
        ...
        <div className="grid md:grid-cols-2 gap-10">
          <motion.div {...fadeInUp} transition={{ delay: 0.2 }} className="space-y-6">
            {[
              ...
              { label: "Age", field: "age", value: "18 years" },
              { label: "Email", field: "email", value: "sarah.anderson@example.com" },
              { label: "Profession", field: "profession", value: "Software Engineer" },
              { label: "Location", field: "location", value: "Ahmedabad, India" },
              { label: "Currency", field: "currency", options: ["USD ($)"] },
              { label: "Country", field: "country", options: ["United States", "India"] },
            ].map(({ label, field, value, options }) => (
              <div key={label}>
                <h2 className="text-xl font-['Poppins'] font-semibold text-[#6B46C1] mb-2">{label}</h2>
                {isEditing && options ? (
                  <select value={userData[field]} onChange={e => handleInputChange(field, e.target.value)} className="w-full p-3 border border-[#E2E8F0] rounded-lg text-[#2D3748] bg-white focus:ring-2 focus:ring-[#F6AD55]">{options.map(opt => <option key={opt}>{opt}</option>)}</select>
                ) : isEditing ? (
                  <input value={userData[field]} onChange={e => handleInputChange(field, e.target.value)} className="text-lg text-[#2D3748] w-full bg-transparent border-b border-[#E2E8F0] focus:outline-none focus:border-[#F6AD55]" />
                ) : (
                  <p className="text-lg text-[#2D3748]">{value || userData[field]}</p>
                )}
              </div>
            ))}
          </motion.div>
        ...
        <motion.div {...fadeInUp} transition={{ delay: 0.8 }} className="flex flex-wrap gap-4 justify-center pb-8 border-t border-[#E2E8F0] pt-8">
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={handleEditToggle} className="flex items-center gap-2 px-8 py-3 bg-[#F6AD55] text-white rounded-xl font-['Poppins'] font-medium hover:bg-[#ED8936] transition-colors duration-300 shadow-md hover:shadow-xl">
            <Settings className="w-5 h-5" />{isEditing ? "Save Profile" : "Edit Profile"}
          </motion.button>
          {[{ i: Share2, t: "Share Profile" }, { i: Download, t: "Download Report" }, { i: Settings, t: "Settings" }].map((b, i) => (
            <motion.button key={i} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="flex items-center gap-2 px-8 py-3 border border-[#E2E8F0] rounded-xl text-[#6B46C1] font-['Poppins'] font-medium hover:bg-[#F7FAFC] transition-all duration-300 shadow-sm">
              <b.i className="w-5 h-5" />{b.t}
            </motion.button>
          ))}
        </motion.div>
      </div>
